...
When displaying a primary and secondary table in a card, use the `variant` property. It increases the left and right table paddings to `--spacing-5x` and remove the bottom border of the last row. Find more details in the [Card layout for table example](../../components/card/card-code#card-layout-for-tables).
