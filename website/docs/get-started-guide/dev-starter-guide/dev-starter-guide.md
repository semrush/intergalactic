...
Perviously, the library entrypoint was provided with `@semcore/ui` npm package. It depends on each component individual package and reexports it. It doesn't work fine with some unique build system like Astro. Each `@semcore/ui` version has very same equivalent version of `@semcore/ui` . `@semcore/ui` is planned to continue publishing until at least Q2 2024.
