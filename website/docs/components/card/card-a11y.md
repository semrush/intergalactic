...
- If the card has a title, make sure that the title has the appropriate heading level. e.g., if your card follows an `<h2>` heading, the title of the card should use an `<h3>` tag. [Refer to our example](./card-code#basic-usage).
- Add an appropriate ARIA landmark role (e.g., `region`) and an `aria-label` to the card if it represents an important part of the page. This allows screen reader users navigate quickly between different parts of the interface. [Refer to our example](./card-code#basic-usage).
