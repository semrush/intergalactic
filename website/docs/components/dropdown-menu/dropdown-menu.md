...
| Maximum height of the list | ![DropdownMenu gets scroll with more than seven items in the list](static/dropdown-height.png)        | The recommended maximum height of the dropdown list with search is seven items. **If there are more items in the list, the dropdown gets scroll and a fader for the next eighth item.** |
...
| Disabled            | ![DropdownMenu list item with disabled state](static/item-disabled.png)              | Use [`--disabled-opacity`](/style/design-tokens/) token. |
