...
tabs: Design('success-message'), A11y('success-message-a11y')
---

## Description

A success message pattern is a response from the system confirming that an action has been completed.
...
* Users should be able to close the success modal window or notice at any time – e.g., with a close button or by pressing the `Escape` key.

e.g., if a [Feedback form](/components/feedback-form/feedback-form) has been sent successfully, show the success message in the same place next to the form's trigger.

![](static/success-dialog.png)
