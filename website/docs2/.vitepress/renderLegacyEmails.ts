...
export const renderLegacyEmails = (tokenList: any[], index: number) => {
  const renderFunc = (tokens: any[], idx: number) => {
    ...
    if (token.type === 'container_legacy_emails_view_open') {
      ...
      const compiledExample = fs.readFileSync(resolvePath(__dirname, '../../../semcore/email/', compiledExamplePath), 'utf-8');
      const sourceExample = fs.readFileSync(resolvePath(__dirname, '../../../semcore/email/', sourceExamplePath), 'utf-8');
      const highlightedSourceCode = markdownRenderer.render('```html' + '\n' + sourceExample + '\n```\n');

      return `<LegacyEmailsView compiledCode="${btoa(encodeURIComponent(compiledExample))}" sourceCode="${btoa(encodeURIComponent(highlightedSourceCode))}">`
    }
