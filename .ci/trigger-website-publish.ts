...
(async () => {
  ...
  log('Commit will be created with following changes:\n');
  log(gitChanges.join('\n') + '\n');
  ...
  if (!confirmation.startsWith('Confirm')) {
    log('Aborted, nothing was affected');
    process.exit(0);
  }
  ...
  if (stashed) {
    ...
    log(picocolors.green('popped stash successfully\n'));
    await git.add(['.']);
  }
  ...
  log(picocolors.green(`added commit "${commitMessage}"`));

  await git.push('origin');
  log(picocolors.green('pushed commit to remote'));

  await git.addTag(tagName);
  log(picocolors.green(`added local git tag "${tagName}"`));
  await git.push('origin', tagName);
  log(picocolors.green(`pushed git tag "${tagName}" to remote`));
  log(picocolors.blue('Checkout website cloud build was started in commits history on GitHub'));
})();
