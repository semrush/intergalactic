...
test.describe('One level Header', () => {
  ...
  test.describe('Multi level Header', () => {
    test('Verify lines props work correctly', async ({ page }) => {
      const standPath = 'stories/components/data-table/tests/examples/header-tests/multi-level-header.tsx';
      const htmlContent = await e2eStandToHtml(standPath, 'en');
      await page.setContent(htmlContent);
      await expect(page).toHaveScreenshot();
  ...
  test.describe('Multi level header - Sorting', () => {
    test('Verify keyboard interactions with sorting', async ({ page }) => {
        const standPath = 'stories/components/data-table/tests/examples/header-tests/multi-level-sorting.tsx';
        const htmlContent = await e2eStandToHtml(standPath, 'en');
        await page.setContent(htmlContent);
