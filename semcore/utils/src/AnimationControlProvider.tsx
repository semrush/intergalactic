...
const useInternalAnimationControlContext = (override: AnimationControl | undefined) => {
  ...
  React.useEffect(() => {
    ...
    const handleMediaQueryChange = () =>
      setValue(mediaQuery?.matches ? 'animations-disabled' : 'animations-enabled');
...
export const WithAnimationControl = <
  Props extends {
...
>(
  ...
  return React.forwardRef((props, ref) => {
    ...
    return <C {...props} animationControl={animationControl} ref={ref} />;
  }) as any;
};

export const AnimationControlProvider: React.FC<{
  children: React.ReactNode;
  disable: 'always' | 'when-reduce-motion-preferred';
}> = ({ children, disable }) => {
  const override = React.useMemo(
    () => (disable === 'always' ? ('animations-disabled' as const) : undefined),
    [disable],
  );
  const animationControl = useInternalAnimationControlContext(override);

  return (
    <animationControlContext.Provider value={animationControl}>
      {children}
