...
test('Verify violet primary comtrol theme', async ({ page }) => {
  ...
  const primaryInfoColor = await parentDiv.evaluate(el => getComputedStyle(el).getPropertyValue('--intergalactic-control-primary-info'));
  const primaryInfoHoverColor = await parentDiv.evaluate(el => getComputedStyle(el).getPropertyValue('--intergalactic-control-primary-info-hover'));
  const primaryInfoActiveColor = await parentDiv.evaluate(el => getComputedStyle(el).getPropertyValue('--intergalactic-control-primary-info-active'));
...
test.describe('Tokens with custom component', () => {
    test('Verify token applies for switch and svg', async ({ page }) => {
      const standPath = 'stories/components/utils/design-tokens/docs/examples/tokens-with-custom-component.tsx';
      const htmlContent = await e2eStandToHtml(standPath, 'en');
      await page.setContent(htmlContent);
