...
test.describe('Scatterplot chart', () => {
  ...
  test('Verify basic usage ', async ({ page }) => {
    const standPath = 'stories/components/d3-chart/tests/examples/scatterplot-chart/basic-usage.tsx';
  ...
  test('Verify scetterplot with colors ans numbers  ', async ({ page }) => {
    const standPath = 'stories/components/d3-chart/tests/examples/scatterplot-chart/color-customization-and-values-inside.tsx';
    const htmlContent = await e2eStandToHtml(standPath, 'en');
    await page.setContent(htmlContent);
  ...
  test('Verify venn legend and pattern fill', async ({ page, browserName }) => {
    ...
    const standPath = 'stories/components/d3-chart/tests/examples/scatterplot-chart/legend-and-pattern-fill.tsx';
    const htmlContent = await e2eStandToHtml(standPath, 'en');
    await page.setContent(htmlContent);
