...
class CigaretteBarRoot extends Component<any> {
  ...
  getRect() {
    const { x, y, width, height, r, direction, index, hovered } = this.asProps;

    let radius = r;

    if (direction === 'vertical' && height < r) radius = height;
    if (direction === 'vertical' && height < r) radius = height;
  ...
  render() {
    ...
    this.asProps.dataHintsHandler.describeValueEntity(`${index}.${x}`, '');
