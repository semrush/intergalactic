...
test.describe('DayPicker with today button', () => {
  ...
  await test.step('Verify date picker popper', async () => {
    ...
    await test.step('Verify hover disabled date', async () => {
      ...
      const backgroundColor = await cells.first().evaluate((el) => getComputedStyle(el).backgroundColor);
    ...
    await test.step('Verify style of available  date', async () => {
      ...
      const backgroundColor = await cells.nth(2).evaluate((el) => getComputedStyle(el).backgroundColor);
    ...
    const cell = page.locator('[data-ui-name="CalendarDays.Unit"][class*="__startSelected_"][class*="__endSelected_"]');
...
test.describe('DayPikcer with trigger and popper', () => {
  ...
  test('Verify mouse nteractions when component uses expanded trigger and popper', async ({ page }) => {
...
const newValue = await input.inputValue();
  ...
  test('Verify keyboard nteractions when component uses expanded trigger and popper', async ({ page }) => {
    const standPath = 'stories/components/date-picker/docs/examples/trigger_and_popper.tsx';
    const htmlContent = await e2eStandToHtml(standPath, 'en');
