...
describe('normalizeCurvePoints', () => {
  test.concurrent(
    'Replace points with nulls with closest non-null variable in the start of array',
    () => {
  ...
  test.concurrent(
    'Replace points with nulls with closest non-null variable in the end of array',
    () => {
  ...
  test.concurrent(
    'Don`t replace points if they are not located in the start or in the end of array',
    () => {
...
describe('filterPoints', () => {
  ...
  test.concurrent('Returns false if point has null', () => {
    const source = addNulls([0], pointsMock);
    expect(source.filter(filterFn)).toHaveLength(0);
  });
  test.concurrent('Returns false if point is first or last element of points array', () => {
    const source = addNulls([0, pointsMock.length - 1], pointsMock);
    expect(source.filter(filterFn)).toHaveLength(0);
  });
  test.concurrent('Returns false if point before or after doesnt have nulls', () => {
    expect(pointsMock.filter(filterFn)).toHaveLength(0);
  });
  test.concurrent('Returns false if only point before or after has hulls', () => {
    const source = addNulls([0, 3], pointsMock);
    expect(source.filter(filterFn)).toHaveLength(0);
  });
  test.concurrent('Returns true if point before has null & point after has null', () => {
    const source = addNulls([0, 2], pointsMock);
    expect(source.filter(filterFn)).toHaveLength(1);
  });

  test.concurrent('Returns true if point start get data & point next has null', () => {
    const source = addNulls([1], pointsMock);
    expect(source.filter(filterFn)).toHaveLength(1);
  });

  test.concurrent('Returns true if point finish get null & point after has null', () => {
    const source = addNulls([18], pointsMock);
    expect(source.filter(filterFn)).toHaveLength(1);
  });

  test.concurrent('Support export function colors', () => {
