...
- `onChange` prop typing for `Checkbox.Value`.
...
- Deprecated atomic types. Atomic parts are part of `NSCheckbox`
namespaces.
...
- Unchecked themed `Checkbox` should remain gray.
...
- Version minor update due to children dependencies update (`@semcore/typography` [16.1.5 ~> 16.2.0], `@semcore/base-components` [16.1.2 ~> 16.2.0]).
