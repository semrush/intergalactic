...
test.describe('Input-tags', () => {
  test('Input with Select - Empty elect focuses every time when user navigates by keyboard', async ({ page , browserName}) => {
    ...
    const x = 150;
    const y = 30;
    await emelent.scrollIntoViewIfNeeded();
    await emelent.click({ position: { x, y } });
    await emelent.click({ position: { x, y } });
    await expect(frame.locator('[data-ui-name="Select.Trigger"]')).toBeVisible();
    await expect(frame.locator('[data-ui-name="Select.Trigger"][class*="keyboardFocused"]')).not.toBeVisible();
    ...
    await expect(frame.locator('[data-ui-name="Select.Trigger"][class*="keyboardFocused"]')).toBeVisible();
    await expect(frame.locator('[data-ui-name="Select.Menu"]')).toBeVisible(); 
    const linkedInOption = await frame.getByRole('option', { name: 'LinkedIn' }).getAttribute('class');
