...
export async function expectAttributes(locator: Locator, attributes: Record<string, string | RegExp>) {
...
await expectAttributes(paletteClose, {
  ...
  test('Styles', async ({ page }) => {
    ...
    const paddings = await getComputedStyles(locators.popper, ['paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft']);
...
await test.step('Verify divider styles', async () => {
    const dividerStyles = await getComputedStyles(locators.divider, ['marginTop', 'marginBottom']);
...
await test.step('Verify input styles', async () => {
    const wrapperStyles = await getComputedStyles(page.locator('[data-ui-name="Input"]'), ['width', 'marginLeft']);
...
await test.step('Verify hover on text color ', async () => {
  ...
  test('Custom colors states ', async ({ page }) => {
    const standPath = 'stories/components/color-picker/docs/examples/several_ways_to_use_component.tsx';
    const htmlContent = await e2eStandToHtml(standPath, 'en');

    await page.setContent(htmlContent);
    const locators = getColorPickerLocators(page);

    await expect(locators.trigger.first()).toHaveAttribute('aria-label', 'Color field, current color is #98848D');
...
await test.step('Verify active state of Palette custom color ', async () => {
    ...
    await expect(locators.trigger.first()).toHaveAttribute('aria-label', 'Color field, current color is #8649E6');
  ...
  test('Verify Keyboard navigation when No palette', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot();
    await page.keyboard.press('Space');
    await expect(locators.trigger).toHaveAttribute('aria-label', 'Color field, current color is #F67CF2');
...
await expect(page).toHaveScreenshot();
  ...
  test('Verify Colors and Palette.Manager props', async ({ page }) => {
    const standPath = 'stories/components/color-picker/tests/examples/colors-and-palette-manager-colors-props.tsx';
    const htmlContent = await e2eStandToHtml(standPath, 'en');
    ...
    const colorItems = page.locator('[data-ui-name="ColorPicker.Colors"] [data-ui-name="ColorPicker.Item"]');
  ...
  const paletteItems = page.locator('[data-ui-name="PaletteManager.Colors"] [data-ui-name="PaletteManager.Item"]');
...
await test.step('Verify paletter manager when defaultColors and onColorsChange pre set', async () => {
  ...
  test('Verify ColorPicker.Item PaletteManager.Item and ColorPicker.Input props', async ({ page }) => {
    const standPath = 'stories/components/color-picker/tests/examples/input-color-and-items-props.tsx';
    const htmlContent = await e2eStandToHtml(standPath, 'en');

    await page.setContent(htmlContent);
    const locators = getColorPickerLocators(page);

    await test.step('Verify ColorPicker.Items editable and pre selected', async () => {
 ...
 await expect(page).toHaveScreenshot();
  ...
  const paletteNonEditableItems = page.locator('[data-ui-name="PaletteManager.Colors"]').first().locator('[data-ui-name="PaletteManager.Item"]');
