...
export function processTokens(config: Theme, prefix: string): ProcessedTokens {
  ...
  function traverse(params: {
    ...
    prefix?: string;
    postfix?: string;
    groupKey: keyof ProcessedTokens;
  }) {
  ...
  (Object.keys(config.semanticTokens) as Array<keyof typeof config.semanticTokens>).forEach(
    (key) => {
      switch (key) {
        case 'durations': {
          traverse({
            node: config.semanticTokens[key],
            path: [],
            prefix: `${prefix}-duration`,
            groupKey: 'semanticTokens',
          });
          break;
        }
        case 'radii': {
