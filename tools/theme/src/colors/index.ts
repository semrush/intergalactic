...
export const baseColors: Record<Colors, ColorScale> = {
  gray: new ColorScale(
    ['oklch(0.94 0.002 180)', 'oklch(0.40 0.004 140)', 'oklch(0.22 0.01 140)'],
    'p3',
  ),
  mint: new ColorScale(
  ...
  blue: new ColorScale(['oklch(0.5879 0.1547 278.41)'], 'p3'),
  'blue-indigo': new ColorScale(
    ['oklch(0.861 0.068 266.551)', 'oklch(0.703 0.154 265.122)', 'oklch(0.575 0.232 263.373)'],
    'p3',
  ),
  ...
  pink: new ColorScale(
    ['oklch(0.90 0.10 330)', 'oklch(0.82 0.18 330)', 'oklch(0.66 0.27 330)'],
    'p3',
  ),
};
...
export const colors = colorNames.reduce<Record<Colors, typeof initLightnessMap>>(
  (acc, colorName) => {
    if (!acc[colorName]) {
    ...
    for (const [key, lightness] of lightnessMap.entries()) {
      ...
      acc[colorName][key] = {
        value: baseColors[colorName].at(l),
        description: initLightnessMap[key].description,
      };
    }
