...
function formatColor(color: Color): string {
  ...
  return aR < 1 ? `oklch(${lR} ${cR} ${hR} / ${aR})` : `oklch(${lR} ${cR} ${hR})`;
}
...
export class ColorScale {
  ...
  constructor(colors: string[], gamut: 'srgb' | 'p3' = 'srgb') {
    ...
    if (skipped > 0) throw new Error(`${skipped} color(s) could not be parsed`);

    const first = stops[0];
    const last = stops.at(-1)!;
    if (lightness(first) > 0) stops.unshift(new Color('oklch', [0, 0, first.coords[2] ?? 0]));
    if (lightness(last) < 1) stops.push(new Color('oklch', [1, 0, last.coords[2] ?? 0]));
  ...
  colorAt(l: number): Color {
    l = clamp(l);
    const segment =
      this._segments.find((s) => l >= s.start && l <= s.end) ?? this._segments.at(-1)!;
