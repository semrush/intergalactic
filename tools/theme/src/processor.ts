...
export const writeIfChanged = async (relativePath: string, content: string) => {
  ...
  try {
    ...
    if (
      ...
      !('code' in err) ||
      err.code !== 'ENOENT' ||
      !err.message.startsWith('ENOENT: no such file')
    ) {
      throw err;
    }
  }
...
const { baseTokens, semanticTokens, highlightsTokens } = processTokens(config, PREFIX);

for (const theme of themes) {
  await writeIfChanged(`lib/${theme}.css`, tokensToCss([...baseTokens, ...semanticTokens]));
  await writeIfChanged(`lib/${theme}.ts`, tokensToJs([...baseTokens, ...semanticTokens]));

  if (highlightsTokens.length > 0) {
    await writeIfChanged(`lib/highlights-${theme}.css`, tokensToCss(highlightsTokens));
...
export async function processStyles() {
  ...
  const projectCssPaths = (
    ...
    if (
      path.split('/').some((pathPart) => ['theme', 'chart', 'email', 'table'].includes(pathPart))
    ) {
      return false;
    }
    return true;
  ...
  await Promise.all(
    projectCssPaths.map((path, index) =>
      writeIfChanged(resolvePath(semcorePath, path), processedCss[index].css),
    ),
  );
...
async function processTokensToDocs(
  usages: Record<string, string[]>,
  tokens: { basic: Token[]; semantic: Token[]; highlight: Token[] },
) {
  ...
  for (const processedToken of [...tokens.semantic, ...tokens.highlight]) {
    ...
    const components = [...new Set((usages[name] ?? []).map((cssPath) => cssPath.split('/')[0]))];
